...
| TM-I012 | Legacy cache (6, B4) | **T**ampering | A committed change never reaches the `FINInstitution` cache, so the legacy serves stale entitlement. The legacy swallows the refresh failure in `catch(Throwable)` | Medium | Refresh triggered; **failure surfaced to the operator and alerted**, never swallowed | ADR-INST-016 §2.2 / FR-INSTC-008, NFR-OPS-I02, D-I17 |
...
| TM-I015 | Update path (3) | **T**ampering | A full-column `INSERT`/`UPDATE` nulls out 30 operational columns (`SRVR_IP`, `GRAMT`, `BSNN_STTS_CKYN` …) this slice does not own | Medium | Writes restricted to the 11 owned columns; test asserts the others unchanged | ADR-INST-016 rule 4 |
...
| TM-I012 | Legacy cache staleness window | ACCEPTED | M | We trigger a refresh and surface failures, but cannot guarantee coherence across legacy instances. Cadence unmeasured | PM (RISK-I03) |
